        ...
        function (require, module, exports) {
          ...
          AccessMode.prototype = {
            ...
            isSharer: function (side) {
              return (
                this.isAdmin(side) ||
                AccessMode._checkFlag(this, side, AccessMode._SHARE)
              );
            },
          ...
          const CBuffer = function (compare, unique) {
            ...
            return {
              ...
              forEach: function (callback, startIdx, beforeIdx, context) {
                ...
                for (let i = startIdx; i < beforeIdx; i++) {
                  callback.call(
                    ...
                    i < beforeIdx - 1 ? buffer[i + 1] : undefined,
                    i
                  );
                }
              },
              ...
              find: function (elem, nearest) {
                const { idx } = findNearest(elem, buffer, !nearest);
                return idx;
              },
   ...
   */
          ...
          const Connection = function (config, version_, autoreconnect_) {
            ...
            function boffReconnect() {
              ...
              const timeout =
                _BOFF_BASE *
                (Math.pow(2, _boffIteration) *
                  (1.0 + _BOFF_JITTER * Math.random()));
              // Update iteration counter for future use
              _boffIteration =
                _boffIteration >= _BOFF_MAX_ITER
                  ? _boffIteration
                  : _boffIteration + 1;
                ...
                poller.onreadystatechange = function (evt) {
                  if (poller.readyState == XDR_DONE) {
                    if (poller.status == 201) {
                      ...
                      _lpURL =
                        url_ + "&sid=" + pkt.ctrl.params.sid + "&created=true";
                      ...
                      if (instance.onDisconnect) {
                        const code =
                          poller.status ||
                          (_boffClosed ? NETWORK_USER : NETWORK_ERROR);
                        const text =
                          poller.responseText ||
                          (_boffClosed
                            ? NETWORK_USER_TEXT
            ...
            return {
              ...
              updTopic: function (topic) {
                ...
                return new Promise((resolve, reject) => {
                  ...
                  req.onsuccess = (event) => {
                    trx
                      .objectStore("topic")
                      .put(serializeTopic(req.result, topic));
                    trx.commit();
                  };
                });
              },
              ...
              readMessages: function (topicName, query, callback, context) {
                ...
                return new Promise((resolve, reject) => {
                  ...
                  trx
                    .objectStore("message")
                    .openCursor(range, "prev").onsuccess = (event) => {
        ...
        function (require, module, exports) {
          ...
          function forEach(line, start, end, spans, formatter, context) {
            ...
            for (let i = 0; i < spans.length; i++) {
              ...
              for (
                let si = i + 1;
                si < spans.length && spans[si].at < span.at + span.len;
                si++
              ) {
          ...
          Drafty.parse = function (content) {
            ...
            lines.map(function (line) {
              ...
              INLINE_STYLES.map(function (style) {
                ...
                spans = spans.concat(
                  spannify(line, style.start, style.end, style.name)
                );
              });
          ...
          (function (global) {
            (function () {
              ...
              if (typeof require != "function") {
                throw new Error(
                  "Unable to load modules: require() is not available."
                );
              }
              ...
              var Tinode = function (config, onComplete) {
                ...
                this.loginSuccessful = (ctrl) => {
                  ...
                  if (ctrl.params && ctrl.params.token) {
                    this._authToken = {
              ...
              Tinode.isCommTopicName = function (name) {
                return (
                  Tinode.isP2PTopicName(name) || Tinode.isGroupTopicName(name)
                );
              };
              ...
              Tinode.isNewGroupTopicName = function (name) {
                return (
                  typeof name == "string" &&
                  (name.substring(0, 3) == TOPIC_NEW ||
                    name.substring(0, 3) == TOPIC_NEW_CHAN)
                );
              };
              ...
              Tinode.prototype = {
                ...
                newGroupTopicName: function (isChan) {
                  return (
                    (isChan ? TOPIC_NEW_CHAN : TOPIC_NEW) +
                ...
                getServerLimit: function (name, defaultValue) {
                  return (
                    (this._serverInfo ? this._serverInfo[name] : null) ||
                    defaultValue
                  );
                },
              ...
              Topic.prototype = {
                ...
                subscribe: function (getParams, setParams) {
                  ...
                  // use "new".
                  return this._tinode
                    .subscribe(this.name || TOPIC_NEW, getParams, setParams)
                    .then((ctrl) => {
                      if (ctrl.code >= 300) {
                ...
                publishMessage: function (pub) {
                  ...
                  if (
                    Drafty.hasEntities(pub.content) &&
                    !pub.head.attachments
                  ) {
                    const entities = [];
                ...
                publishDraft: function (pub, prom) {
                  ...
                  prom = (prom || Promise.resolve()).then(
                    ...
                    (err) => {
                      this._tinode.logger(
                        "WARNING: Message draft rejected",
                        err
                      );
                      pub._sending = false;
                      pub._failed = true;
                ...
                leave: function (unsub) {
                  ...
                  if (!this._subscribed && !unsub) {
                    return Promise.reject(
                      new Error("Cannot leave inactive topic")
                    );
                  }
                ...
                latestMessage: function (skipDeleted) {
                  ...
                  if (
                    !skipDeleted ||
                    !msg ||
                    msg._status != MESSAGE_STATUS_DEL_RANGE
                  ) {
                    return msg;
                  }
                ...
                msgStatus: function (msg, upd) {
                  ...
                  if (upd && msg._status != status) {
                    msg._status = status;
                    this._tinode._db.updMessageStatus(
                      this.name,
                      msg.seq,
                      status
                    );
                ...
                _processMetaSub: {
                  value: function (subs) {
                    ...
                    subs.forEach((sub) => {
                      ...
                      sub.updated = new Date(sub.updated);
                      sub.touched = sub.touched
                        ? new Date(sub.touched)
                        : undefined;
                      sub.deleted = sub.deleted ? new Date(sub.deleted) : null;
                      sub.online = !!sub.online;
                ...
                delCredential: {
                  value: function (method, value) {
                    if (!this._subscribed) {
                      return Promise.reject(
                        new Error(
                          "Cannot delete credential in inactive 'me' topic"
                        )
                      );
                    }
                    // Send {del} message, return promise
